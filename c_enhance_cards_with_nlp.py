...
def log_next_line(lines=1, level=logger.info):
    ...
    for i in range(1, lines + 1):
        level(
            linecache.getline(__file__, inspect.getlineno(
                inspect.currentframe()) + i)
        )
...
logger.info(linecache.getline(
    __file__, inspect.getlineno(inspect.currentframe()) + 1))
t = cards_df_pop_parts["pop"].apply(
    lambda x: re.findall(patt, str(x)) if re.findall(
        patt, str(x)) else numpy.nan
)
...
symbols_explanation = {
    ...
    "{R/P}": {
        "explanation": "can be paid with either {R} or 2 life",
        "example_card": "Rage Extractor",
    },
    "{Q}": {
        "explanation": "{Q} is the untap symbol",
        "example_card": "Order of Whiteclay",
    },
    ...
    "{CHAOS}": {
        "explanation": "It is only in Plane cards and for a specific kind of game",
...
for sym in symbols_set_valid:
    if not sym in ["{T}", "{Q}", "mana"]:
        ...
        # # Mana patterns to match in spacy 3.0
        # patterns = [[{"LOWER": sym.lower()}]]
        # # The attributes to assign to the matched token
        # attrs = {"ORTH": sym, "POS": NOUN}
        ...
        nlp.tokenizer.add_special_case(
            sym, [{ORTH: sym, POS: VERB, TAG: "VB"}])

        # # Mana patterns to match
        # patterns = [[{"UPPER": sym.upper()}]]
        # # The attributes to assign to the matched token
        # attrs = {"ORTH": sym, "TAG": "VB", "POS": VERB}
        # # Add rules to the attribute ruler
        # ruler.add(patterns=patterns, attrs=attrs)
        ...
        for label, start, end in matches:
            ...
            for old_span in spans:
                if spans_overlaps(span, old_span):
                    longest, shortest = get_longest_shortest_spans(
                        span, old_span)
                    while shortest in spans:
                        spans.remove(shortest)
                    while longest in spans:
                        spans.remove(longest)
...
# +
zones = ["graveyard", "play", "library",
         "hand", "battlefield", "exile", "stack"]
...
entity_to_kind_map = {}
entity_key_to_hash_map = key_dependent_dict(
    entity_key_hash
)  # entity key: entity node hash (node_id)
...
for typ in cards_types:
    ...
    dict_label_terms[key].append([{"LOWER": t + "s"}
                                 for t in typ.lower().split()])
...
for zone in zones:
    key = "ZONE: " + zone.lower()
    dict_label_terms[key].append(
        [{"LOWER": t, "POS": NOUN} for t in zone.lower().split()]
    )
    dict_label_terms[key].append(
        [{"LOWER": t + "s", "POS": NOUN} for t in zone.lower().split()]
    )
...
for player in players:
    ...
    dict_label_terms[key].append(
        [{"LOWER": t, "POS": spacy.symbols.PRON}
            for t in player.lower().split()]
    )
    dict_label_terms[key].append(
        [{"LOWER": t, "POS": spacy.symbols.NOUN}
            for t in player.lower().split()]
    )
...
for sym in ["counter", "card"]:
    ...
    dict_label_terms[key].append(
        [{"LOWER": t, "POS": NOUN} for t in sym.split()])
...
def parse_doc_to_list_of_dicts(df_row, original_cols=[], doc_col="part_doc"):
    ...
    for t in doc:
        ...
        if t.ent_type_:
            ...
            entity_node_ent_type.append(numpy.nan)
            entity_node_entity.append(numpy.nan)
            entity_node_desc.append(numpy.nan)
            entity_node_lemma.append(numpy.nan)
    ...
    try:
        if not res[res["token_head_dep"] == "root"].index.empty:
            res.loc[
                res[res["token_head_dep"] == "root"].index, "token_head_node"
            ] = res["part_node"]
            res.loc[res[res["token_head_dep"] == "root"].index, "label"] = part_dic[
                "part"
            ]
...
pop_defining_cols = ["card_id", "paragraph_order",
                     "pop_order", "pop_type", "pop"]
part_defining_cols = [
    "card_id",
    "paragraph_order",
    "pop_order",
...
for i in tqdm(range(unique_card_ids.shape[0] // chunksize)):
    ...
    if not ((i + 1) * chunksize) % (10 * chunksize):
        clear_output()
    logger.info(
        "Working on {0} from {1}".format(
            (i + 1) * chunksize, unique_card_ids.shape[0])
    )

    this_batch_ids = unique_card_ids[i * chunksize: (i + 1) * chunksize]
    cards_df_for_graph = cards_df_pop_parts[
        cards_df_pop_parts["card_id"].isin(this_batch_ids)
    ].copy()
    cards_df_for_graph.loc[:, "part_doc"] = cards_df_for_graph["part"].apply(
        lambda x: nlp(x.strip(".,"))
    )

    # logger.info(linecache.getline(__file__, inspect.getlineno(inspect.currentframe()) + 1))
    # Parse for graph
    cards_df_for_graph = cards_df_for_graph.apply(
        parse_doc_to_list_of_dicts, args=(cards_df_pop_parts.columns,), axis=1
    )
    cards_df_for_graph = pd.concat(cards_df_for_graph.values, sort=False).reset_index(
        drop=True
    )
    ...
    cdfg = cards_df_for_graph.merge(
        cards_df, left_on=["card_id"], right_index=True)
    ...
    nodes["token"] = (
        cdfg[nodes_cols["token"]]
        .rename(columns={"token_node": "node_id"})
        .dropna(subset=["node_id"])
    )
    nodes["token"]["type"] = "token"
    nodes["token"]["label"] = nodes["token"].apply(
        lambda x: "-".join(
            [x["token_node_text"], x["token_node_pos"], x["token_node_tag"]]
        ),
        axis=1,
    )
    nodes_attr["token"] = [
        x for x in nodes["token"].columns if x not in ["node_id"]]
    ...
    nodes["entity"] = (
        cdfg[nodes_cols["entity"]]
        .dropna(subset=["entity_node_ent_type"])
        .rename(columns={"entity_node": "node_id"})
    )
    nodes["entity"]["type"] = "entity"
    nodes["entity"]["label"] = nodes["entity"].apply(
        lambda x: "-".join(
            [
                x["entity_node_entity"],
            ]
        ),
        axis=1,
    )
    nodes_attr["entity"] = [
        x for x in nodes["entity"].columns if x not in ["node_id"]]
    ...
    nodes["part"] = (
        cdfg[nodes_cols["part"]]
        .rename(columns={"part_node": "node_id"})
        .dropna(subset=["node_id"])
    )
    nodes["part"]["type"] = "part"
    nodes["part"]["label"] = nodes["part"].apply(
        lambda x: "-".join([x["part"]]), axis=1
    )
    nodes_attr["part"] = [
        x for x in nodes["part"].columns if x not in ["node_id"]]
    ...
    nodes["card"] = nodes["card"].dropna(
        subset=["node_id", "card_name"], how="any")
    nodes["card"]["type"] = "card"
    nodes["card"]["label"] = nodes["card"].apply(
        lambda x: "-".join([x["card_name"]]), axis=1
    )
    nodes_attr["card"] = [
        x for x in nodes["card"].columns if x not in ["node_id"]]
    ...
    edges["token_to_head_part"] = (
        cdfg[edges_cols["token_to_head_part"]]
        .rename(columns=renamer)
        .dropna(subset=["source", "target"], how="any")
    )
    edges["token_to_head_part"]["type"] = "token_to_head_part"
    edges["token_to_head_part"]["label"] = edges["token_to_head_part"].apply(
        lambda x: "-".join(
            [
                x["token_head_dep"],
            ]
    ...
    edges_attr["token_to_head_part"] = [
        x for x in edges["token_to_head_part"].columns if x not in ["source", "target"]
    ]
    ...
    edges["entity_to_token"] = (
        cdfg[edges_cols["entity_to_token"]].dropna().rename(columns=renamer)
    )
    edges["entity_to_token"]["type"] = "entity_to_token"
    edges["entity_to_token"]["relation"] = "is_class_of"
    edges["entity_to_token"]["label"] = edges["entity_to_token"].apply(
        lambda x: "-".join(
            [
                x["relation"],
            ]
    ...
    edges_attr["entity_to_token"] = [
        x for x in edges["entity_to_token"].columns if x not in ["source", "target"]
    ]
    ...
    edges["part_to_card"] = (
        cdfg[edges_cols["part_to_card"]]
        .rename(columns=renamer)
        .dropna(subset=["source", "target"], how="any")
    )
    edges["part_to_card"]["type"] = "part_to_card"
    edges["part_to_card"]["label"] = edges["part_to_card"].apply(
        lambda x: "-".join(
            [
    ...
    edges_attr["part_to_card"] = [
        x for x in edges["part_to_card"].columns if x not in ["source", "target"]
    ]

    # logger.info(linecache.getline(__file__, inspect.getlineno(inspect.currentframe()) + 1))
    # Build dfs
    nodes_df = pd.concat(nodes.values(), sort=True).drop_duplicates(
        subset=["node_id"])
    nodes_df = nodes_df.dropna(
        subset=[x for x in nodes_df.columns if not x in ["node_id", "label"]], how="all"
    )
    edges_df = pd.concat(edges.values(), sort=True).drop_duplicates(
    ...
    nodes_df.set_index(["card_id", "card_name", "node_id", "type"]).to_sql(
        out_nodes_table_name, engine, if_exists=method
    )
    edges_df.set_index(["source", "target", "card_id", "type"]).to_sql(
        out_edges_table_name, engine, if_exists=method
    )
...
# cdfg=cards_df_for_graph
# cards_df_for_graph.merge(cards_df, left_on=['card_id'], right_index=True)
cdfg = cards_df
...
nodes_card_df = cdfg[
    ["card_id", "supertypes", "types", "subtypes", "colors", "manaCost"]
].copy()
...
for col in nodes_card_df:
    ...
    temp["entity_node_entity"] = temp.apply(
        lambda x: ": ".join([x["entity_node_ent_type"], x[col].lower()]), axis=1
    )
    temp["entity_node"] = temp["entity_node_entity"].apply(
        lambda x: entity_key_to_hash_map[x]
    )
...
if card_as_start:
    pass  # Ignore in this case, because we want it reversed: card as target
#     renamer = {'card_id':'source', 'entity_node':'target'}
edges["entity_to_card"] = (
    cdfg[cdfg["edge_type"] == "entity_to_card"][edges_cols["entity_to_card"]]
    .dropna()
    .rename(columns=renamer)
)
edges["entity_to_card"]["type"] = "entity_to_card"
edges["entity_to_card"]["relation"] = "is_contained_in"
edges["entity_to_card"]["label"] = edges["entity_to_card"].apply(
    lambda x: "-".join(
        [
            x["relation"],
        ]
...
edges_attr["entity_to_card"] = [
    x for x in edges["entity_to_card"].columns if x not in ["source", "target"]
]
...
nodes_df.set_index(["card_id", "card_name", "node_id", "type"]).to_sql(
    in_nodes_table_name, engine, if_exists=method
)
edges_df.set_index(["source", "target", "relation", "type"]).to_sql(
    in_edges_table_name, engine, if_exists=method
)
...
# + deletable=false editable=false run_control={"frozen": true}
# res.pivot_table(values=['cont'], index=['ent_node', 'label', 'pop_type'], columns=['edge_type'], aggfunc=np.sum)

...
#     print_every = 10
    ...
    for i, card_id in enumerate(ids_to_process):
        ...
        nodes_set = set(numpy.union1d(
            card_0_edges['source'].values, card_0_edges['target'].values))
        ent_0_nodes_qr = '''
        SELECT * FROM {0}
        WHERE node_id IN {1}
        AND type='entity'
        ...
        # Build graph
        edge_attr = [x for x in card_0_edges.columns if not x in [
            'source', 'target']]
        ...
        for k in card_0_nodes['type'].unique():
            ...
            # pdb.set_trace()
            node_col = 'node_id'
            cols = [x for x in card_0_nodes[card_0_nodes['type'] == k]
                    if x not in ['node_id']]
            for node_attr in cols:
                ...
                nx.set_node_attributes(G, pd.Series(
                    temp[node_attr].values, index=temp[node_col].values).copy().to_dict(), name=node_attr)
        ...
        # Build graph
        edge_attr = [x for x in card_0_in_edges.columns if not x in [
            'source', 'target']]
        ...
        for k in card_0_in_nodes['type'].unique():
            ...
            # pdb.set_trace()
            node_col = 'node_id'
            cols = [x for x in card_0_in_nodes[card_0_in_nodes['type']
                                               == k] if x not in ['node_id']]
            for node_attr in cols:
                ...
                # Eliminate and wrap in quotes
                #temp[node_attr] = temp[node_attr].apply(eliminate_and_wrap_in_quotes)
                nx.set_node_attributes(H, pd.Series(
                    temp[node_attr].values, index=temp[node_col].values).copy().to_dict(), name=node_attr)

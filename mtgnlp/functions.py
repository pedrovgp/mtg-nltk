...
def build_graphs_of_cards(obj_tuple, method="append"):
    ...
    # create logger'
    logger = logging.getLogger("functions_build_graphs_of_cards")
    ...
    for i, card_id in enumerate(ids_to_process):
        ...
        if not i:
            logger.debug(f"{f_id}: loop started")
        ...
        """.format(
            "public." + out_nodes_table_name, card_id
        )
        ...
        OR target IN {1}
        """.format(
            "public." + out_edges_table_name,
            "(" + ",".join(["'" + x + "'" for x in card_0_nodes["node_id"]]) + ")",
        )
        ...
        nodes_set = set(
            numpy.union1d(card_0_edges["source"].values, card_0_edges["target"].values)
        )
        ent_0_nodes_qr = """
        SELECT * FROM {0}
        WHERE node_id IN {1}
        AND type='entity'
        """.format(
            "public." + out_nodes_table_name,
            "(" + ",".join(["'" + x + "'" for x in nodes_set]) + ")",
        )
        ...
        for k in card_0_nodes["type"].unique():
            ...
            cols = [
                x
                for x in card_0_nodes[card_0_nodes["type"] == k]
                if x not in ["node_id"]
            ]
            for node_attr in cols:
                ...
                nx.set_node_attributes(
                    G,
                    pd.Series(temp[node_attr].values, index=temp[node_col].values)
                    .copy()
                    .to_dict(),
                    name=node_attr,
                )
        ...
        SELECT * FROM {0} WHERE card_id = '{1}'
        """.format(
            "public." + in_nodes_table_name, card_id
        )
        ...
        OR target IN {1}
        """.format(
            "public." + in_edges_table_name,
            "(" + ",".join(["'" + x + "'" for x in card_0_in_nodes["node_id"]]) + ")",
        )
        ...
        nodes_set = set(
            numpy.union1d(
                card_0_in_edges["source"].values, card_0_in_edges["target"].values
            )
        )
        ent_0_nodes_qr = """
        SELECT * FROM {0}
        ...
        # Build graph
        edge_attr = [
            x for x in card_0_in_edges.columns if not x in ["source", "target"]
        ]
        ...
        for k in card_0_in_nodes["type"].unique():
            ...
            cols = [
                x
                for x in card_0_in_nodes[card_0_in_nodes["type"] == k]
                if x not in ["node_id"]
            ]
            for node_attr in cols:
        ...
        if (not i % X) and i:
            logger.debug(
                f"{f_id}: Exporting to Postgres. Elapsed: {datetime.datetime.now()-start}"
            )
            method = method
            ...
            df.to_sql(
                cards_graphs_as_json_to_table,
                engine,
                if_exists=method,
                dtype={
                    "outgoing": sqlalchemy.types.JSON,
                    "incoming": sqlalchemy.types.JSON,
                },
            )
            result = {}
            logger.debug(
                f"{f_id}: Exporting to Postgres finished. Elapsed: {datetime.datetime.now()-start}"
            )

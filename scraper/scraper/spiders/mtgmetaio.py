...
from scrapy.linkextractors import LinkExtractor  # https://docs.scrapy.org/en/latest/topics/link-extractors.html#topics-link-extractors
...
class QuotesSpider(CrawlSpider):
    ...
    def parse_deck_url(self, response):
        ...
        self.log(f'Deck parsing: {response.url}')

        result_to_export = {}
        
        # deck identifiers
        result_to_export['deckname'] = response.selector.xpath('//h1[contains(@class, "deckname")]/text()').get()
        ...
        # Cards in deck
        decklist = response.selector.xpath('//ul[contains(@class, "sampledecklist")]/li')
        # cards_in_deck is a list of dicts
        result_to_export['cards_in_deck'] = [b.attrib for b in decklist if b.attrib.get('data-name', False)]
        ...
        # deck identifiers
        deckvs = response.selector.xpath('//ul[contains(@class, "deckvs")]/li[@data-perf]')
        vs_results = []
        for d in deckvs:
            d_selector = Selector(text=d.get())
            attrib_dict = d.attrib
            attrib_dict['matches'] = d_selector.xpath('//span[contains(@class, "matches")]/text()').get()
            attrib_dict['vs_deck_id'] = d_selector.xpath('//a[has-class("btn")]/@href').get()
